class MessagesController < ApplicationController
  ...
  def create
    ...
    if sender != receiver
      ...
      into_db = Message.strong_message_insert(batch_id, sender_id, sender_name, receiver_id, receiver_name, title, content, redis_mail) 
      #into_db = Message.strong_mail_insert(batch_id, sender_id, sender_name, sender_image, receiver_id, receiver_name, receiver_image, title, content, redis_mail) 
      if into_db
  ...
  def inbox
    ...
    if not mails
      mails = MessageInbox.select('redis_mail').where(:user_id => current_user.id)
      #mails = MultiJson.encode(mails)
  ...
  def renew
    mail = Message.where(:id => params[:id])
